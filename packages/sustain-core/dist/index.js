...
__export(index_exports, {
  AdvancedSustainabilityAnalyzer: () => AdvancedSustainabilityAnalyzer,
  CarbonEnergyAnalyzer: () => CarbonEnergyAnalyzer,
  ComposeAnalyzer: () => ComposeAnalyzer,
  DockerResourceCollector: () => DockerResourceCollector,
  ProjectAnalyzer: () => ProjectAnalyzer,
  ProjectFutureAnalyzer: () => ProjectFutureAnalyzer,
...
var ProjectAnalyzer = class {
  ...
  async validateProjectPath() {
    try {
      const stats = await import_fs.promises.stat(this.projectPath);
  ...
  async checkVulnerableDependencies(issues) {
    ...
    try {
      ...
      const auditPromise = new Promise((resolve2) => {
        var _a2;
        try {
          (0, import_child_process.execSync)("npm audit --json", { cwd: this.projectPath, stdio: "pipe" });
          resolve2(null);
        } catch (e) {
  ...
  async analyzeSanity() {
    const issues = [];
    try {
      if (!await this.fileExists((0, import_path.join)(this.projectPath, "README.md"))) {
        issues.push({
          type: "missing-readme",
          file: "README.md",
      ...
      if (!await this.fileExists((0, import_path.join)(this.projectPath, ".gitignore"))) {
        issues.push({
      ...
      if (await this.fileExists((0, import_path.join)(this.projectPath, ".env"))) {
        if (!await this.fileExists((0, import_path.join)(this.projectPath, ".env.example"))) {
          issues.push({
  ...
  async checkPackageJson(issues) {
    ...
    try {
      const packageJson = JSON.parse(await import_fs.promises.readFile(packageJsonPath, "utf8"));
      if (!packageJson.name) {
        issues.push({
  ...
  async getAllCodeFiles() {
    ...
    return files.filter((file) => codeExtensions.includes((0, import_path.extname)(file)));
  }
  async hasTestFiles() {
    try {
      const files = await this.getAllFiles();
...
var ComposeAnalyzer = class {
  ...
  async findProjectRoot(startPath = this.projectPath) {
    let currentPath = (0, import_path2.resolve)(startPath);
    const indicators = ["package.json", ".git", "pnpm-workspace.yaml", ".gitignore"];
    while (currentPath !== "/" && currentPath !== (0, import_path2.resolve)(currentPath, "..")) {
      for (const indicator of indicators) {
        try {
          await import_fs2.promises.access((0, import_path2.join)(currentPath, indicator));
          console.log(`Found project root at: ${currentPath}`);
          return currentPath;
        } catch {
        }
      }
      currentPath = (0, import_path2.resolve)(currentPath, "..");
    }
    return null;
  }
  async analyzeComposeFile(filename) {
    const filePath = filename.startsWith("/") ? filename : (0, import_path2.join)(this.projectPath, filename);
